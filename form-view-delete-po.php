...
?>
    ...
    <h6><font color="orange" size="5"><u>Delete PO</u></font>       <u>
    <left></left>
    </u></h6>
<form action="home-buyer.php?page=form-view-delete-po" method="post" name="postform">
...
if(isset($_POST['cari'])){
	...
	if(empty($po_number) ){
		$Tampil=mysql_query("select * from 
							...
							(select po_number, po_date, customer_id, file_name_detail from purchase_order_header where po_number not in (select po_number from faktur_jual))a,
		...
		$jumlah=mysql_fetch_array(mysql_query("select count(po_number)total from 
											(select a.po_number, a.po_date, a.customer_id, b.customer_name, c.qty, c.total from 
											(select po_number, po_date, customer_id from purchase_order_header where po_number not in (select po_number from faktur_jual))a,
											(select customer_id, customer_name from master_customer)b,
											(select po_number, sum(qty) as qty, sum(total) as total from purchase_order_detail group by po_number)c
											where a.po_number = c.po_number and a.customer_id = b.customer_id)a"));
...
<table width="1206" border="0" align="center" cellpadding="0" cellspacing="1">
    ...
    <tr align="center">
        ...
        <a href="home-buyer.php?page=proses-delete-po&po_number=<?=$po_number?>" onclick="return confirm('are you sure you want to delete the PO number <?=$po_number?> !!!')"> Delete </a></div></td>
    </tr>
    <tr>
